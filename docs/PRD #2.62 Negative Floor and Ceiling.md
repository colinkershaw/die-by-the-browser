...
| Input             | Parses as                                                                       |
|-------------------|---------------------------------------------------------------------------------|
| `4d6--3`          | filter: drop-low 1 (no modifier — filter fires)                                 |
| `4d6+-3`          | filter: keep-low 1 (no modifier — filter fires)                                 |
| `3d6-18--6`       | modifier -18, floor -6 (filter stage skipped — `token[idx+1]` was `1`, a digit) |
| `3d6-18+-3`       | modifier -18, ceiling -3 (same)                                                 |
| `3d6-18--6+-3`    | modifier -18, floor -6, ceiling -3                                              |
| `4d6--1-18--6+-3` | all together: filter: drop low 1; modifier -18, floor -6, ceiling -3            |
...
| `4d6--3`          | filter: drop-low 3, no modifier *(existing behaviour unchanged)*   |
| `4d6+-3`          | filter: keep-low 3, no modifier *(existing behaviour unchanged)*   |
| `4d6--3-18--6+-3` | filter: drop-low 3; modifier -18, floor -6, ceiling -3             |
| `4d6+-3-18--6+-3` | filter: keep-low 3; modifier -18, floor -6, ceiling -3             |
...
- Parse `4d6--3-18--6+-3`, reconstruct formula, assert it equals `"4d6--3-18--6+-3"`. Parse reconstructed formula, assert collections are identical to original parse.
- Parse `4d6+-3-18--6+-3`, reconstruct formula, assert it equals `"4d6+-3-18--6+-3"`. Parse reconstructed formula, assert collections are identical to original parse.
...
- Limits without a modifier (e.g. a bare floor on a plain `3d6`)
  - People can workaround with a zero (0) modifier, eg `3d6+0-1+5`
